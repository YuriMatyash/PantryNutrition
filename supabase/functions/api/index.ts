...
import { handleDeleteRecipe, handleEditRecipe, handleGetRecipe, handleListRecipes } from "./routes/recipes.ts";
...
Deno.serve(async (req: Request) => {
  ...
  if (req.method === "POST" && /^\/api\/recipes\/[^/]+\/edit$/.test(url.pathname)) {
    return handleEditRecipe(req, url.pathname, origin);
  }
